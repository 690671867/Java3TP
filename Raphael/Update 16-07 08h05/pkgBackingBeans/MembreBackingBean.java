...
public class MembreBackingBean implements Serializable {
	...
	public ArrayList<Membre> getListeFavoris() {
		listeFavoris=MembreManager.getListeFavoris(membre.getMembreId());
	...
	public String verifierLogin()
	{
		...
		if(membre!=null){
			//peopler le user
			membre.setListeFavoris(FavorisManager.getFavorisByMemberId(membre.getMembreId()));
			membre.setListeHobbies(HobbyManager.getListeHobbiesByMembreId(membre.getMembreId()));
			membre.setListeMessages(MessageManager.getListeMessagesRecusByMemberId(membre.getMembreId()));
			membre.setListePhotos(PhotoManager.getPhotosByMemberId(membre.getMembreId()));
			int online=MembreManager.setUserOnline(membre.getMembreId());
			session=(HttpSession) FacesContext.getCurrentInstance().getExternalContext().getSession(true);
