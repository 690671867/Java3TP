...
public class RechercheBackingBean implements Serializable {
	...
	public String ajouterMembre(Membre membre)
	{
		...
		if(listeFavoris.size()<maxFavoris){
			//ajouter amis
			int ajoute=MembreManager.ajouterFavoris(mbb.getMembre(), membre);
			
			if(ajoute>0){
				//envoyer courriel a qui a ete ajoute
				SendEmail.envoyerMail(mbb.getMembre(), membre, "ajoute");
			}
		}
		return destination;
	}
	
	public String envoyerClinDoeil(Membre membre)
	{
		String destination=null;
		int envoye=0;
		envoye=ClinsdoeilManager.envoyerClinDoeil(mbb.getMembre(), membre);
